...
def best_orders(infile):
    files = expand("{files}", files = [i.split("/")[1] for i in open(infile).read().splitlines()])
    return files
...
rule trimming:
    input:
        "ordermarkers/bestlikelihoods.txt"
    output:
        trimmed_files = expand("ordermarkers/best.trimmed/trimmed.{trimfile}", trimfile = best_orders("ordermarkers/bestlikelihoods.txt")),
        done = "trim.done"
    params:
        trim_threshold = "10"
    log:
