...
##########################################################
#
#  Lep-Anchor wrapper, version 3 (allopolyploid version)
#
#  usage: lepanchor_wrapper3.sh -f ref.fasta -e extra_cut_site_file -n num_chr -c chain_file -p paf_file -r prox_file -m map_file1 -m map_file2, ... 
#
...
echo "##########################################################"
echo "#"
echo "#  Lep-Anchor wrapper3 (allopolyploid)"
echo "#"
echo "#  usage: lepanchor_wrapper3.sh -T thread_per_run -t threads -f ref.fasta -e extra_cut_site_file -n num_chr -c chain_file -r prox_file -p paf_file -m map_file1 -m map_file2, ... "
echo "#"
...
then
	...
	PARALLEL="parallel --tmpdir . --jobs $THREADS"
fi
...
do
cat $i
done|sort -V -k 1,1 -k 2,2n >map_all_sorted


#CleanMap
echo "running CleanMap..."
$JAVA -cp $LABIN CleanMap map=map_all_sorted >map_all.clean
...
for i in $(seq $CHR)
do
echo "gunzip -fc $CHAIN|$JAVA -cp $LABIN PlaceAndOrientContigs numThreads=$THREADS2 bed=map_extra.bed chromosome=$i map=$MAP chain=- paf=$PAF proximity=$PROX keepEmptyIntervals=0 >chr$i.la 2>chr$i.la.err"
done|$PARALLEL
...
for i in $(seq $CHR)
do
echo "gunzip -fc $CHAIN|$JAVA -cp $LABIN PlaceAndOrientContigs numThreads=$THREADS2 $(awk -f pickorientation.awk chr$i.la) bed=map_extra_prop.bed chromosome=$i map=$MAP chain=- paf=$PAF proximity=$PROX keepEmptyIntervals=1 >ichr$i.la 2>ichr$i.la.err"
done|$PARALLEL
...
for i in $(seq $CHR)
do
echo "gunzip -fc $CHAIN|$JAVA -cp $LABIN PlaceAndOrientContigs numThreads=$THREADS2 $(awk -f pickorientation.awk chr$i.la) bed=map_extra_prop2.bed chromosome=$i map=$MAP chain=- paf=$PAF proximity=$PROX keepEmptyIntervals=1 evaluateAnchoring=prop$i.la improveAnchoring=1 numRuns=1 >iichr$i.la 2>iichr$i.la.err"
done|$PARALLEL

echo "finding full haplotypes..."
awk '($NF=="haplotype")' iichr*.err|sort -n -r|awk -vlimit=2000 '($NF=="haplotype" && ($1>=($4-$3+1-limit)/limit) && (!(($5 SUBSEP $6 SUBSEP $7) in h))){h[$2,$3,$4]; print}' >fullHaplotypes50.txt
...
awk -f removeHaplotypes.awk map_extra_prop2.bed fullHaplotypes50.txt > map_extra_prop2_nh.bed

#PlaceAndOrientContigs
echo "running PlaceAndOrientContigs (fourth iteration)..."
for i in $(seq $CHR)
do
echo "gunzip -fc $CHAIN|$JAVA -cp $LABIN PlaceAndOrientContigs numThreads=$THREADS2 $(awk -f pickorientation.awk chr$i.la) bed=map_extra_prop2_nh.bed chromosome=$i map=$MAPL chain=- paf=$PAF proximity=$PROX keepEmptyIntervals=1 evaluateAnchoring=iichr$i.la improveAnchoring=1 numRuns=1 >iiichr$i.la 2>iiichr$i.la.err"
done|$PARALLEL
...
for i in $(seq $CHR)
do
        awk -f prune.awk iiichr$i.la >iiichr${i}_pruned.la
done 2>pruned.la

#remove overlap(s)
awk -f removeOverlaps.awk map_extra_prop2.bed iiichr*_pruned.la >iiall.la
...
cut -f 1 contigs.length|grep -v -w -F -f <(cut -f 2 fullHaplotypes50.txt;cut -f 1 iiall.la) >not_used_final.txt
...
do
	for c in $(seq $CHR)
	do
	awk -vn=$c '($3==n)' $m|awk -f liftover.awk REF_LA.agp -|awk -vm=$j '(/LG/ && NF>=4){if (NF==4) {print $1"\t"$2"\t"$3"\t"m"\t"$4"\t"$4} else for (i=4;i<NF;i+=2) print $1"\t"$2"\t"$3"\t"m"\t"$(i)"\t"$(i+1)}'
	done
	j=$[$j + 1]
