...
#  Lep-Anchor wrapper
#
#  usage: lepanchor_wrapper.sh -f ref.fasta -n num_chr -e extra_cut_site_file -c chain_file -p paf_file -r prox_file -m map_file1 -m map_file2, ... 
#
...
echo "#"
echo "#  usage: lepanchor_wrapper.sh -t threads -f ref.fasta -n num_chr -e extra_cut_site_file -c chain_file -r prox_file -p paf_file -m map_file1 -m map_file2, ... "
echo "#"
...
while getopts ":e:n:c:p:m:f:t:r:" OPTION; do
	case ${OPTION} in
	t)
	THREADS=$OPTARG;;
	n)
...
awk -f propagate.awk chr*.la >tmp1.la
...
#awk -f propagate2.awk tmp$i.la|awk '(/^[^#]/ && NF>=8){++d[$1 "\t" $7+0 "\t" $8+0]; data[++line]=$0}END{for (i = 1; i <= line; ++i) {$0=data[i];if (d[$1 "\t" $7+0 "\t" $8+0] == 1) fn="prop"$5".la"; else if ($5==1) fn="prop0.la"; else fn=""; if (fn != "") print $0>fn}}'
...
for i in $(seq $CHR)
do
        awk -f prune.awk ichr$i.la >ichr${i}_pruned.la
done 2>pruned.la
