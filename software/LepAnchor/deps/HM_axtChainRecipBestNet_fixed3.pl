...
## V3
## for bb_mis_join, 在某个情况下, chainPreNet 提示needLargeMem: trying to allocate 0 bytes (limit: 4294967296), 然后错误退出
##  处理该问题是暂时吧所有chainPreNet剔除
...
sub rbestNet {
	...
	$cmd  = "gunzip -c $in_chain_file ";
	#$cmd .= "| chainPreNet /dev/stdin $target.sizes $query.sizes /dev/stdout 2>>$path/_ChainPreNet.log "; ##  处理该问题是暂时吧第一个chainPreNet剔除
	...
	$cmd  = "cat $path/$query.$target.$net_name.tbest.chain ";
	...
	$cmd  = "cat $path/$target.$query.$net_name.rbest.chain ";	
	$cmd .= "| chainNet -minSpace=$min_space -minScore=$min_score /dev/stdin $target.sizes $query.sizes /dev/stdout /dev/null 2>>$path/_ChainNet.log ";
	...
	$cmd .= "$target.seq $target.seq $path/$target.$query.$net_name.rbest.axt"; ###########fixing , both are target seq
	print $cmd,"\n";
	system($cmd);
	# axt to chain
	my $options;
