#calculate scores both ways, to the next and previous contig at the same time
#awk -f propagate2 chr[0-9]*.la
BEGIN{
	if (scoreDiff == "")
...
function processLine(line, mode)
{
	...
	if (mode == 1) {
		if (count[contig] > 1) {
			...
			#contig is a bridge between two other contigs...			
			if (count[prevContig] == 1 && count[nextContig] == 1 && $11 + $14 + $15 >= score[contig,$5])
				score[contig,$5]=$11 + $14 + $15
