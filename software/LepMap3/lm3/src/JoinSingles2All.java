...
public class JoinSingles2All {
	public static void main(String args[])
	{
		...
		pp.warning(new String[]{"minLod","map", "data", "lodLimit", "informativeMask", "femaleTheta", "maleTheta", "sizeLimit", "theta", "lodDifference", "lod3Mode", "betweenSameType", "numThreads", "distortionLod", "iterate", "maxDistance", "mask"});
		...
		String maskFile = pp.getValueAsString("mask", null);
		if (maskFile != null) {
			LGMap2 mask = new LGMap2();
			if (!mask.loadLGMap(maskFile))
				Error.error(302);
			sc.maskLG(mask, 1);
		}
		
		sc.joinSingles(lodLimit, Double.parseDouble(pp.getValueAsString("lodDifference", "0.0")), map, pp.getValueAsString("betweenSameType", "0").equals("1"), pp.getValueAsString("iterate", "0").equals("1"), Integer.parseInt(pp.getValueAsString("numThreads", "1")), Integer.parseInt(pp.getValueAsString("maxDistance", "" + Integer.MAX_VALUE)));
